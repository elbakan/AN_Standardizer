  ...
  *        a nonstandardized field would return -1	*/
	...
	for(k = 0;k <= passstan;k++) {
	  ...
	  if ((AddrRes = (struct standardized_address *)Addr_Stan2(teststring,' ','u',"  ","     ")) != NULL) {
	    ...
	    sprintf(outstring[iout],"%s                                                                   %46.46s%4.4s%07d",fixedstring,standname[k],a_filename,written++);
	    outstring[iout++][newlength] = 0;
	  }
	}
     } else {
	if ((AddrRes = (struct standardized_address *)Addr_Stan2(teststring,' ','u',"  ","     ")) != NULL) {
	  sprintf(sparestring[kout],"%s%6.6s%5.5s%20.20s%10.10s%4.4s%5.5s%3.3s%5.5s%4.4s%5.5s%46.46s",fixedstring
		,AddrRes->hn
		,AddrRes->snpd
		,AddrRes->msn
		,AddrRes->snst
